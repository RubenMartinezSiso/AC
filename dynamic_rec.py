...
class MyApplication():
    ...
    def sumhrec(self, aqueducto, long1, HEIGHT, XS, YS):
        """sumhrec: calculate summation of hi recursive"""
        if long1 < 0:
            return 0
        elif aqueducto[long1] == 1:
            return HEIGHT - YS[long1] + self.sumhrec(aqueducto, long1-1, HEIGHT, XS, YS)
        return 0 + self.sumhrec(aqueducto, long1-1, HEIGHT, XS, YS)

    def sumdrec(self, aqueducto, long2, XS, YS):
        ...
        while index > 0 and not exit:
            if aqueducto[index] == 0:
                ...
                exit = True
        return (XS[long2] - XS[index])**2 + self.sumdrec(aqueducto, index, XS, YS)
    ...
    def calculatecost(self, aqueduct, HEIGHT, XS, YS, ALPHA, BETA):
        """calculatecost: calculate the cost of the entire aqueduct received"""
        lenght = len(aqueduct)
        sumh = self.sumhrec(aqueduct, lenght-1, HEIGHT, XS, YS)
        sumd = self.sumdrec(aqueduct, lenght-1, XS, YS)
    ...
    def generate(self, stack, height, NUMBER, XS, YS, ALPHA, BETA):
        ...
        if stack2.size() != NUMBER:
            ...
            if not self.impossiblestack(stack2.content(), height, XS[0], YS[0], XS, YS):
                if stack2.size() <= 2:
                    minicost = self.calculatecost(stack2.content(), height, XS, YS, ALPHA, BETA)
                    LAST = minicost
                    MATRIX.append(minicost)
                else:
                    temporal = stack2.content()
                    rest = LAST
                    minicost = self.calculatecost(temporal, height, XS, YS, ALPHA, BETA)
                    LAST = minicost
                    minicost = minicost - rest
                    COUNTER += 1
                    MATRIX.append(minicost)
            ...
            while index <= COUNTER:
                ...
                index += 1
            if not self.impossiblestack(stack2.content(), height, XS[0], YS[0], XS, YS) and othercost < PROVISIONALCOST:
