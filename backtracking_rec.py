...
def calculatecost(acueducto):
    ...
    sumh = sumhrec(acueducto, NUMBER-1, HEIGHT)
    sumd = sumdrec(acueducto, NUMBER-1)
...
def generate(pila, height):
    ...
    if copia.longitud() != NUMBER:
        ...
        if not impossiblestack(copia.contenido(), height, XS[0], YS[0]) and newcost < PROVISIONALCOST:
